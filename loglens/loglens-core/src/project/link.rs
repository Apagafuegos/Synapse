...
// software projects and their LogLens configurations in the global registry.
...
/// - Project's .loglens/metadata.json contains project_id
...
pub async fn link_project(project_path: Option<&Path>) -> Result<LinkResult> {
    ...
    if !loglens_dir.exists() {
        bail!(
            "No .loglens directory found at {}. Run 'loglens init' first.",
...
///
/// This removes the registry entry but preserves the .loglens/ directory
...
pub async fn unlink_project(project_path: Option<&Path>) -> Result<UnlinkResult> {
    ...
    if !metadata_path.exists() {
        bail!(
            "No LogLens metadata found at {}. Project may not be initialized.",
            project_path.display()
        );
    }
...
mod tests {
    ...
    async fn test_link_without_init() {
        ...
        assert!(result.unwrap_err().to_string().contains("Run 'loglens init' first"));
    }
    ...
    async fn test_unlink_project() {
        ...
        // Verify .loglens directory still exists
        assert!(project_path.join(".loglens").exists());
    }
