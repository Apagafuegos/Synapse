...
impl Config {
    /// Get the unified Synapse data directory
    ...
    fn get_config_path() -> Option<PathBuf> {
        ...
        if let Ok(current_dir) = env::current_dir() {
            let project_config = current_dir.join(".synapse.toml");
        ...
        if let Some(home_dir) = dirs::home_dir() {
            let user_config = home_dir.join(".config").join("synapse").join("config.toml");
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let config_path = Self::get_config_path()
            .unwrap_or_else(|| {
                ...
                if let Some(home_dir) = dirs::home_dir() {
                    home_dir.join(".config").join("synapse").join("config.toml")
                } else {
