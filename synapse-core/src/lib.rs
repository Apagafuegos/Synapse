// Synapse Library - Core log analysis functionality
//
...
// Old MCP server implementation removed - use synapse-mcp crate instead
...
/// Analyze log lines using the default Synapse configuration
pub async fn analyze_lines(
    ...
    let synapse = match Synapse::new() {
        Ok(synapse) => {
            debug!("Synapse instance created successfully");
            synapse
        }
        Err(e) => {
            error!("Failed to create Synapse instance: {}", e);
            return Err(e);
        }
    };

    match synapse.analyze_lines_with_model(raw_lines, level, provider_name, api_key, selected_model).await {
        Ok(result) => {
...
/// Process an MCP request using the default Synapse configuration
pub async fn process_mcp_request(request_json: &str) -> Result<String> {
    debug!("Processing MCP request");
    let synapse = match Synapse::new() {
        Ok(synapse) => synapse,
        Err(e) => {
            error!("Failed to create Synapse instance for MCP request: {}", e);
            return Err(e);
        }
    };
    ...
    match synapse.process_mcp_request(request).await {
        Ok(response) => {
...
impl Synapse {
    ...
    /// Create a Synapse instance with custom config
...
impl Default for Synapse {
    fn default() -> Self {
        Self::new().expect("Failed to create Synapse with default config")
    }
}