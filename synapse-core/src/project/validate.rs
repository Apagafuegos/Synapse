...
pub async fn validate_project(project_path: &PathBuf) -> Result<ProjectValidation> {
    ...
    if !synapse_dir.exists() {
        validation.synapse_exists = false;
        validation.errors.push("No .synapse directory found".to_string());
        return Ok(validation);
    }
    validation.synapse_exists = true;
...
pub fn format_issue_type(issue_type: &IssueType) -> &'static str {
    match issue_type {
        ...
        IssueType::ConfigDirectoryMissing => ".synapse directory not found",
...
mod tests {
    ...
    async fn test_validate_valid_project() {
        ...
        assert!(validation.synapse_exists);
    ...
    #[tokio::test]
    async fn test_validate_missing_synapse() {
        let temp_dir = TempDir::new().unwrap();
        let project_path = temp_dir.path().to_path_buf();
    ...
    async fn test_validate_links_with_issues() {
        ...
        registry.register_project(
            ...
            PathBuf::from("/nonexistent/.synapse"),
        ).unwrap();
