...
//
// This module manages the global registry of Synapse projects at
// ~/.config/synapse/projects.json, enabling persistent bidirectional
// linkage between software projects and their Synapse configurations.
...
pub struct RegistryEntry {
    ...
    /// Absolute path to .synapse/ directory
    pub synapse_config: PathBuf,
...
impl ProjectRegistry {
    /// Get the global registry file path (~/.config/synapse/projects.json)
    pub fn registry_path() -> Result<PathBuf> {
        ...
        fs::create_dir_all(&config_dir)
            .context("Failed to create Synapse config directory")?;
    ...
    pub async fn validate(&self) -> Vec<ValidationIssue> {
        ...
        for (project_id, entry) in &self.projects {
            ...
            if !entry.synapse_config.exists() {
                issues.push(ValidationIssue {
                    project_id: project_id.clone(),
                    issue_type: IssueType::ConfigDirectoryMissing,
                    path: entry.synapse_config.clone(),
                });
            } else {
                // Verify metadata matches
                let metadata_path = entry.synapse_config.join("metadata.json");
...
mod tests {
    ...
    fn test_registry_create_and_save() {
        ...
        let synapse_config = root_path.join(".synapse");
